...
public class HouseController {
	...
	public String show(@PathVariable(name = "id") Integer id,
		...
		Favorite favorite = null; // favorite を null にしておく
		// ログイン済ならレビューが投稿できる
		if (userDetailsImpl != null ) {
			// ユーザー情報の取得
			User user = userDetailsImpl.getUser();
			// そのユーザーのレビューの有無をチェックするメソッドをセット
			hasUserAlreadyReviewed = reviewService.hasUserAlreadyReviewed(house, user);
			// 1.そのユーザーのお気に入りの有無をチェックするため、まずisFavoriteへfavoriteServiceのisFavoriteメソッド(引数house, user)をセット
			isFavorite = favoriteService.isFavorite(house, user);
			// 2. 分岐文を作成、isFavoriteがtrue（民宿に対してのお気に入りが存在している）の場合
	        if (isFavorite) {
	            // 3. favoriteへfavoriteRepositoryのfindByHouseAndUserメソッド(引数house, user)をセット
