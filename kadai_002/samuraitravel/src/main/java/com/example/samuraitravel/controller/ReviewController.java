...
public class ReviewController {
	...
	public String index(@PathVariable(name = "houseId") Integer houseId,
		...
		// それぞれのmodelに "house"・"reviewPafw" をキーとして house・reviewPage の属性を追加
	...
	// 対象となる宿のレビューを投稿フォームの表示
	@GetMapping("/register")
	public String register(@PathVariable(name = "houseId") Integer houseId, Integer rating, String comments, Integer userId,
			               Model model) {
		
		House house = houseRepository.getReferenceById(houseId);
        
		model.addAttribute("house", house);
        model.addAttribute("reviewRegisterForm", new ReviewRegisterForm(rating, comments, houseId, userId));
